 ...
 * @author admin
 */
public class LoadDB4 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>4.Radio x Radio</title>");
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet LoadDB1 at " + request.getContextPath() + "</h1>");
            dao.LoadStd();
            dao.LoadDept();
            ...
            for (Student st : dao.getStd()) {
                ...
                tbl += "<td>" + st.getDob() + "</td>";
                tbl += "<td>" + st.getGpa() + "</td>";
                tbl += "<td><a href='LoadDB4?type=0&id=" + st.getId() + "'>Update</a></td>";
                tbl += "<td><a href='javascript:void(0)' onclick='confirmDelete(" + st.getId() + ", \"" + st.getName() + "\", \"LoadDB4\")'>Delete</a></td>";
                tbl += "</tr>";
            }
            tbl += "</table>";
            //Gender: checkbox  ;  Department: select
            String form = "<form action='LoadDB4' method='post'>\n"
                    ...
                    + "'><br/>\n"
                    + "            Gender: <input type='radio' name='chkGender' value='M'"
                    + (stUpdate != null?(stUpdate.isGender() ? "checked" : ""):"")
                    + ">Male\n"
                    + "            <input type='radio' name='chkGender' value='M'"
                    + (stUpdate != null?(stUpdate.isGender() ? "" : "checked"):"")
                    + ">Female<br/>\n"
                    + "            Department: <br/>\n";
            for (Department de : dao.getDept()) {
                form += "                <input type='radio' name='sltDept' value='" + de.getId() + "' "
                        + (stUpdate != null && de.getId().equals(stUpdate.getDepartId()) ? "checked" : "")
                        + ">" + de.getName() + "<br/>\n";
            }
            form += "            Dob: <input type='date' name='datDob' value='"
