 ...
 * @author admin
 */
public class LoadDB1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>1.Radio x Combo-box</title>");
            ...
            for (Student st : dao.getStd()) {
                ...
                tbl += "<td>" + st.getDob() + "</td>";
                tbl += "<td>" + st.getGpa() + "</td>";
                tbl += "<td><a href='LoadDB1?type=0&id=" + st.getId() + "'>Update</a></td>";
                tbl += "<td><a href='javascript:void(0)' onclick='confirmDelete(" + st.getId() + ", \"" + st.getName() + "\", \"LoadDB1\")'>Delete</a></td>";
                tbl += "</tr>";
            }
            tbl += "</table>";
            //Gender: checkbox  ;  Department: select
            String form = "<form action='LoadDB1' method='post'>\n"
